...
**CRITICAL:** **Before generating a single token of the result, you must execute the following logic:**
    * **Index:** Mentally list every unique Success Criterion, Constraint, and Requirement found in the User'sinput_data.
...
* **Architecture Enforcement:** `IF` the User Input contains monolithic paragraphs, complex code blocks, nested requirements, raw workflows, or non-linear logic, `THEN`:
...
* **Completeness:** STRICTLY PROHIBIT placeholders (e.g., `// TODO`, `...rest of code`). All logic must be fully implemented.
